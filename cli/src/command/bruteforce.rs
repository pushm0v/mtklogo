...
pub fn run_bruteforce(path: PathBuf, slots: Option<Vec<usize>>, output: PathBuf, width: u32) -> Result<()> {
    println!("{} file {}, max width {}, saving to {}",
             cmd("bruteforce"),
    ...
    for (id, blob) in image.blobs.iter().enumerate() {
        ...
        if should_extract {
            // Try each width from 1 to max width
            for test_width in 1..=width {
                match extract_logo(id, blob, test_width, &output) {
                    Err(e) => println!(
                        "{} {} with width {} : {}",
                        warn("Could not bruteforce slot"),
                        data1(id),
                        data1(test_width),
                        err(e.description())),
                    _ => (),
...
fn extract_logo(id: usize, blob: &Vec<u8>, width: u32, outpath: &PathBuf)
    ...
    let extract = |mode: &ColorMode| -> Result<()>{
        ...
        let filename = format!("bruteforce_{}_{}x{}_", id, width, height) + &info.filename();
